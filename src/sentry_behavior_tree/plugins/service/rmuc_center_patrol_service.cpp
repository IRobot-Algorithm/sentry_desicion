#include <string>
#include "sentry_behavior_tree/plugins/service/rmuc_center_patrol_service.hpp"

namespace sentry_behavior_tree{

    RmucCenterPatrolService::RmucCenterPatrolService(const std::string & service_node_name,
        const BT::NodeConfiguration & conf)
        : nav2_behavior_tree::BtServiceNode<sentry_srvs::srv::NavGoal>(service_node_name, conf){}

    void RmucCenterPatrolService::on_tick()
    {
        ...
        switch (id_)
        {
            ...
            default:
            {
                ...
                request_->pose.pose.position.y = 2.7;
                break;
            }

        }   
        // RCLCPP_INFO(node_->get_logger(),"rmuc_center_patrol_service on_tick()... ");
    }

    BT::NodeStatus RmucCenterPatrolService::check_future(
...
{
  factory.registerNodeType<sentry_behavior_tree::RmucCenterPatrolService>(
    "RmucCenterPatrol");
    //注意这里""内没有Service 
}